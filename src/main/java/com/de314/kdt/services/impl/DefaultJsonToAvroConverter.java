...
import com.de314.kdt.services.AvrifyConverter;
import com.de314.kdt.services.JsonToAvroConverter;
import com.de314.kdt.utils.Opt;
import com.de314.kdt.utils.StreamUtils;
...
public class DefaultJsonToAvroConverter implements JsonToAvroConverter, AvrifyConverter {
    ...
    public Object convert(String json, String schemaStr) {
        log.debug("Converting: {}", json.replaceAll("\n", " "));
        json = avrify(json, schemaStr);
        log.debug("Avrified to: {}", json.replaceAll("\n", " "));
        ...
        log.debug("Final Model: {} - {}", datum.getClass().getSimpleName(), datum);
        return datum;
    }
    ...
    private JsonNode avrify(JsonNode message, Schema schema) {
        ...
        if (message.isObject() && isRecord(schema, true)) {
            ...
            message.elements().forEachRemaining(ele -> {
                ...
                JsonNode newField = ele == null ? NullNode.getInstance() : avrify(ele, fs);
    ...
    private JsonNode convertUnion(JsonNode message, Schema schema) {
        ...
        switch (message.getNodeType()) {
                ...
                if (unionTypes.contains(Schema.Type.ENUM)) {
                    ...
                    switch (validEnumSchemas.size()) {
                        ...
                        default:
                            ...
                            log.warn("Matched multiple enums: {} => {}",
                                    message.asText(),
            ...
            case NUMBER:
                ...
                } else if ((message.isInt() || message.isLong()) && unionTypes.contains(Schema.Type.LONG)) {
                    mappedObject.put("long", message.asLong());
                } else if (unionTypes.contains(Schema.Type.DOUBLE) || unionTypes.contains(Schema.Type.FLOAT)) {
    ...
    public Schema guessSchema(JsonNode message, List<Schema> validSchemas) {
        ...
        switch (message.getNodeType()) {
            ...
            case OBJECT:
                ...
                } else if (typeMap.containsKey(Schema.Type.MAP)) {
                    List<Schema> mapSchemas = typeMap.get(Schema.Type.MAP);
                    switch (mapSchemas.size()) {
                        case 1:
                            bestGuess = mapSchemas.get(0);
                            break;
                        default:
                            throw new RuntimeException("Found multiple matching schemas");
                    }
